...
public class Controller {
    ...
    private Game game;
    private HighScoreWindow hsWindow;
    ...
    private void setUpRollButtonListener(){
        window.getYatzyPanel().getRollButton().addActionListener(l -> {
            changeButtonStates(true);
            if(window.getYatzyPanel().getRollButton().getText().equals("Avsluta")){
                window.setNewStartPanel();
                window.getStartPanel().getNameField().setText(game.getPlayerName());
                setUpMenuListeners();
                window.changePanelTo(window.getStartPanel());
            }
    ...
    private void setUpHighscoreButtonListener(){
        window.getYatzyPanel().getShowScoreButton().addActionListener(l -> {
            if(hsWindow != null){
                hsWindow.dispose();
            }
            hsWindow = new HighScoreWindow(game.database.getListOfScores());
        });
    }

    private void setUpSelectedDieColorListener(){
        for (JToggleButton diceButton : window.getYatzyPanel().getDiceButtons()) {
